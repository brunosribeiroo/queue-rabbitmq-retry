...
async function publish(queue, content, options = {}) {
    if (!pubChannel) {
    ...
    amqpConn.createChannel(function (err, ch) {
        ...
        function processMsg(msg) {
            ...
            try {
                if (ok) {
                    ch.ack(msg);
                    console.log('[listener] Message successfully consumed')
                    return true;
                } else {
                    // Checks if the message has already tried to be consumed  
                    if (msg.properties == undefined || msg.properties.headers == undefined || !msg.properties.headers["x-death"]) {
                        ...
                        // Checks how many times the message has tried to be processed
                        ...
                        if (counter >= config.retry) {
                            console.log("[listener] Message consumption retry limit reached");
                            ch.ack(msg);
                            return true;
                        } else {
                            ch.nack(msg, false, false);
